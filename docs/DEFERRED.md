...
- **`IRBuilder` reference invalidation under `create_block`.** `IRBuilder` keeps a `BasicBlock* current_block_` pointer and returns `BasicBlock&` references from `create_block()` / `new_block()`. Both point into `fn.blocks` (a `std::vector<BasicBlock>`). When `create_block` does `push_back`, the vector may reallocate — invalidating `current_block_` AND any previously-returned references (e.g. `then_bb` and `merge_bb` in `lower_if` after the second `create_block`). Subsequent `emit()` calls then write into dangling memory, and the affected blocks come out empty.
  - Surfaced by spec 008's recursive-Fibonacci probe: `fn check(n: int) -> int { if (n < 2) { return 99; } return 7; }` returns 7 for any input because the `then` branch's instructions never make it into the IR.
  - Affected files: `include/ir/builder.hpp` (`current_block_`, `set_insert_point`, `create_block`), `src/ir/lowering.cpp` (`lower_if`, `lower_while` — both hold `BasicBlock&` references across multiple `create_block` calls).
  - Suggested fix: change `IRBuilder` to address the current block by *index* into `fn.blocks` (parallel to spec 008's interpreter fix). `create_block()` returns a `BlockHandle` (index wrapper) or a `uint32_t` block id; `set_insert_point(BlockHandle)` updates the index. The lowering pass holds handles, not references.
  - Severity: silent miscompile of any control-flow construct. Any source program with `if` or `while` produces wrong-but-not-crashing IR. The bug has been latent because no existing test exercised control flow inside a user function end-to-end through the interpreter.
